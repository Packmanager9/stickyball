...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Sticky{
        constructor(x,y){
            ...
            this.max = 20
            for(let t = 0;t<200;t++){
                let node = new Circle(Math.random()*1280, 200+Math.random()*300, 6, "cyan")
                node.angle = 0
                node.dis = 0
                node.active = 0
                this.nodes.push(node)
        ...
        draw(){
            ...
            for(let t = 0;t<this.nodes.length;t++){
                this.nodes[t].draw()
                if(this.body.doesPerimeterTouch(this.nodes[t])){
            ...
            for(let t = 0;t<this.nodes.length;t++){
                for (let k = 0; k < this.activenodes.length; k++) {
                    if (this.activenodes[k].doesPerimeterTouch(this.nodes[t])) {
                        let link = new LineOP(this.nodes[t], this.body)
                    if(link.hypotenuse()>this.max){
                        this.max = link.hypotenuse()
                    }
            ...
            // this.angle+= (y-this.body.y)*.07
                ...
                for(let t = 0;t<ramps.length;t++){
                    while(ramps[t].doesPerimeterTouch(this.body)){
                        this.body.y--
                        this.body.ymom = 0
                        j++
                        if(j > 100){
            ...
            for(let t = 0;t<this.activenodes.length;t++){
                ...
                while(ramps[r].doesPerimeterTouch(this.activenodes[t])){
                    this.body.y--
                    this.body.ymom = 0
